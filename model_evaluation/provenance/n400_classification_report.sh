...
def run_and_save_models(df, iteration):
    ...
    def get_model(df, num_suppliers, total_rows=400): #total_rows is the default number of rows taken for the function.
    ...
    for i in range(1, 6): #iterates 5 times with the i taking the values 1 to 5 iteratively
        ...
        model_path = f"/ei/.project-scratch/4/4e0cae75-e749-4340-a118-93a3d5555375/seed_classifier/trained_models/n400/m{i}_{iteration}.json"
...
def evaluate_models(num_iterations, species_to_int, report_filename='n400_classification_report.csv'):
